...
# Only load workbench if GUI is available (not in freecadcmd/console mode)
if not FreeCAD.GuiUp:
    FreeCAD.Console.PrintMessage("AICopilot: GUI not available, skipping workbench initialization\n")
    # Early exit - don't define workbench or start services in console mode
    # The rest of this file requires FreeCADGui which doesn't exist in console mode
    __all__ = []  # Export nothing
else:
    # GUI is available, proceed with normal initialization
    import FreeCADGui
    ...
    # Check for test mode - skip auto-start during tests
    if os.environ.get('FREECAD_MCP_TEST_MODE') == '1':
        FreeCAD.Console.PrintMessage("⚠️  Test mode detected - AI Copilot auto-start skipped\n")
    else:
