...
8. **Reference tracking exists, but is version-gated and partial.** `DepEdge` (`src/App/DepEdge.h`) plus `getInListProp()`/`getOutListProp()` on `DocumentObject` give a structured, property-level dependency graph — `FromObj`, `FromProp`, `ToObj`, `ToProp`, exposed to Python via `DepEdgePy`. Confirmed fed by expression bindings specifically (`ExpressionEngine.getLinksProp()` → `DocumentObject.cpp:457`), so VarSet references are captured. **Python-side, this is exposed as the `InListProp` PROPERTY (plain attribute access, e.g. `obj.InListProp`), not as a callable `getInListProp()` method** — FreeCAD's binding generator strips the `get` prefix here despite the C++ implementation being named `DocumentObjectPy::getInListProp`; confirmed live 2026-08-31 via `dir(obj)` against a real FreeCAD 26.3.0 instance after the original implementation's `hasattr(obj, 'getInListProp')`/`obj.getInListProp()` calls were found to silently always report "unavailable" on every FreeCAD build, real bug not a version gate. Two caveats:
   - **Version gate:** introduced by commit `626689ceb7` ("Core: Introduce dependency edges", 2025-11-19); the earliest FreeCAD tag containing it is `weekly-2026.06.24`. It is **absent from every 1.1.x stable release** (1.1.0 through 1.1.3, checked via `git tag --contains`). This repo's CI targets `1.1-stable` (the 1.1.1 AppImage) — `list_references` must feature-detect (`hasattr(obj, 'InListProp')`) and degrade honestly, not crash, on that target.
...
- **Found by live verification, not code/spec review (2026-08-31, full-review follow-up):** `list_references` called `varset_obj.getInListProp()` as a method — the exact spelling of the C++ implementation (`DocumentObjectPy::getInListProp`, `DocumentObjectPyImp.cpp:350`) — but FreeCAD's Python binding generator actually exposes it as the `InListProp` **property** (`obj.InListProp`, no parens, no `get` prefix). `hasattr(obj, 'getInListProp')` was therefore always `False`, on every real FreeCAD instance regardless of version — this was never a version-gate hit correctly; the "unavailable" branch fired unconditionally, everywhere, since the feature shipped. Neither the original C++-source-reading review pass nor 51 mocked unit tests could catch this, since `MockVarSet` faithfully reproduced the same wrong name. Confirmed the correct name via `dir(obj)` against a live FreeCAD 26.3.0 instance, fixed the handler/mock/tests together, and confirmed live: `remove_property`'s reference-blocking safety check and `list_references`'s dependency-edge listing both now actually work against real FreeCAD.
