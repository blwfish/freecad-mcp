...
| `assembly_operations` | Assembly workbench (Phase 1 — no joints yet): create an `Assembly::AssemblyObject` container, create Local Coordinate System mating references, add components as lightweight links (`App::Link`/`Assembly::AssemblyLink`, same- or cross-document — cross-document requires the source document already open, and both the source and active document already saved to disk, since FreeCAD's cross-document links need a file path on both ends), list an assembly's components. |
