...
1. **`inspect()` resolves the same paths identically across FreeCAD 1.1.1, weekly builds, and `FC-clone`'s locally-built 26.3.0-dev.** *Status: still not directly verified across all three — this session only tested against `FC-clone`'s 26.3.0-dev build.* Cross-version consistency (1.1.1 AppImage vs. weekly vs. local build) remains an open check before relying on the snapshot comparison across CI's two slots.
